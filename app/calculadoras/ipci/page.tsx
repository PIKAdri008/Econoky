...
const clampNumber = (valor, min, max) => {
...
const sanitizeCurrencyInput = (valor, max) => {
...
export default function IPCIPage() {
  ...
  const clampGasto = (valor) => sanitizeCurrencyInput(valor, 100000000)

  const clampYear = (valor, fallback) =>
  ...
  const calcularIPCI = () => {
    ...
    // Calcular totales para IPCI ponderado

    const totalAnterior = resultadosProductos.reduce((sum, p) => sum + p.precioAnterior, 0)

    const totalActual = resultadosProductos.reduce((sum, p) => sum + p.precioActual, 0)

    // IPCI General Ponderado (correcto)

    const ipciPonderado = ((totalActual - totalAnterior) / totalAnterior) * 100
 
    // Variación Promedio (promedio simple de todos los IPCI individuales)
...
<h1 className="text-4xl font-bold mb-4 text-gray-900">Calculadora de IPCI</h1>
...
<div>
<label className="block text-sm font-medium text-gray-900 mb-1">Año Anterior</label>
<input
...
<div>
<label className="block text-sm font-medium text-gray-900 mb-1">Año Actual</label>
<input
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-4">Productos y Servicios</h2>
...
<label className="block text-sm font-medium text-gray-900 mb-2">{producto.nombre}</label>
...
<button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-3 rounded-full font-semibold transition-colors"
>
...
<button
              ...
              className="px-6 py-3 rounded-full border border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors"
>
...
<h3 className="text-2xl font-bold text-gray-900 mb-4">IPCI General (Ponderado)</h3>
<div className="flex items-center gap-4">
<div className={`text-5xl font-bold ${getColorVariacion(resultados.ipciPonderado)}`}>

                  {resultados.ipciPonderado > 0 ? '+' : ''}{resultados.ipciPonderado.toFixed(2)}%
</div>
<div>

                  {getIconoVariacion(resultados.ipciPonderado)}
</div>
</div>
<p className="text-sm text-gray-600 mt-2">

                Variación real de tu gasto total entre {añoAnterior} ({formatoEUR.format(resultados.totalAnterior)}) 

                {' '}y {añoActual} ({formatoEUR.format(resultados.totalActual)})
</p>
<p className="text-xs text-gray-500 mt-1 italic">

                Este es el IPCI correcto, ponderado por el peso de cada gasto en tu presupuesto
</p>
</div>
 
            {/* VARIACIÓN PROMEDIO */}
<div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg">
<h3 className="text-2xl font-bold text-gray-900 mb-4">Variación Promedio Simple</h3>
<div className="flex items-center gap-4">
...
<p className="text-sm text-gray-600 mt-2">
                ...
                Media aritmética de las variaciones de {resultados.totalProductos} producto{resultados.totalProductos !== 1 ? 's' : ''} 

                {' '}entre {añoAnterior} y {añoActual}
</p>
<p className="text-xs text-gray-500 mt-1 italic">

                Este cálculo no refleja el impacto real en tu economía (todas las categorías pesan igual)
</p>
</div>
 
            {/* Resultados por Producto */}
<div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg">
<h3 className="text-xl font-bold text-gray-900 mb-4">IPCI por Producto</h3>
<div className="overflow-x-auto">
<table className="min-w-full text-sm">
<thead className="text-left text-gray-600 uppercase tracking-wide text-xs border-b border-gray-200">
<tr>
...
<td className="py-3 font-medium text-gray-900">{producto.nombre}</td>
...
<span className="text-gray-700">Incremento de precio</span>
</div>
<div className="flex items-center gap-2">
<TrendingDown className="w-4 h-4 text-green-500" />
<span className="text-gray-700">Reducción de precio</span>
</div>
<div className="flex items-center gap-2">
<Minus className="w-4 h-4 text-gray-500" />
<span className="text-gray-700">Sin variación</span>
</div>
</div>
</div>
...
<h4 className="font-bold text-gray-900 mb-2">ℹ Información sobre el IPCI</h4>
<p className="text-sm text-gray-700 mb-3">

                El <strong>IPCI General (Ponderado)</strong> es el cálculo correcto que refleja el impacto real 

                en tu economía. Calcula cuánto ha variado tu gasto total considerando el peso de cada categoría 

                en tu presupuesto.
</p>
<p className="text-sm text-gray-700">

                La <strong>Variación Promedio Simple</strong> es solo una media aritmética de los porcentajes 

                individuales donde todos los productos pesan igual, sin considerar su importancia real en tu gasto. 

                Este dato es útil para ver tendencias generales, pero no refleja tu situación económica real.
</p>
</div>
</div>
